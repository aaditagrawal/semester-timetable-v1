...
import { PlusIcon, TrashIcon, XIcon, CheckIcon, PencilSimpleIcon } from "@phosphor-icons/react";
...
function SetupModalImpl({
  ...
  const [selections, setSelections] = useState<UserElectiveSelections>(initialSelections || {});
  ...
  const [lookupState, setLookupState] = useState<"idle" | "loading" | "done" | "empty" | "error">(
    "idle",
  );
  ...
  const handleLookup = async () => {
    ...
    try {
      ...
      const { lookupAllocation, normalizeRegistration } = await import("@/lib/allocation-lookup");
  ...
  const showLookup =
    step === "start" || (isEditing && step === "electives" && unsetProgramElectives.length > 0);
    ...
    <AlertDialog open={open}>
      <AlertDialogContent className="max-w-[calc(100vw-2rem)] sm:max-w-md md:max-w-lg p-3 sm:p-4 max-h-[90dvh] overflow-hidden flex flex-col">
                ...
                <p className="text-[11px] text-muted-foreground">
                  {step === "start" ? (
                    <>
                      ...
                      You&apos;ll get to check them next.
                    </>
                  ) : (
                    <>
                      {unsetProgramElectives.length} program elective
                      {unsetProgramElectives.length > 1 ? "s are" : " is"} still unset. Fill{" "}
                      {unsetProgramElectives.length > 1 ? "them" : "it"} from your registration
                      number — anything you have already chosen is kept.
                    </>
                  )}
                </p>
        ...
        <div className="flex-shrink-0 pt-2">
          ...
          <AlertDialogFooter>
            {isEditing && onClose && (
              <AlertDialogCancel onClick={onClose} size="sm">
                Cancel
              </AlertDialogCancel>
            )}
