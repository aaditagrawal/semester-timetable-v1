...
export function ThemeProvider({ children }: { children: React.ReactNode }) {
  ...
  const setMode = useCallback((mode: ThemeMode) => commit(settingsRef.current, mode), [commit]);
  ...
  const setAccent = useCallback(
    (hex: string) =>
      commit({ ...settingsRef.current, templateId: null, accent: hex }, modeRef.current),
    [commit],
  );
  ...
  const resetTheme = useCallback(() => commit(DEFAULT_THEME, modeRef.current), [commit]);
