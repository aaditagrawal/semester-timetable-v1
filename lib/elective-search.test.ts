...
function referenceSearchOptions(options: ElectiveOption[], query: string): ElectiveOption[] {
...
const QUERIES = (() => {
  ...
  for (const option of ALL) {
    ...
    for (let i = 1; i <= Math.min(option.code.length, 8); i += 1)
      queries.add(option.code.slice(0, i));
  }
  return [...queries];
})();
