...
import { buildOptionIndex, buildOptionsByType, resolveSelections } from "@/lib/elective-index";
...
export function useTimetable() {
  ...
  const addCustomElective = useCallback(
    (elective: CustomElective) => commitCustomElectives([...customElectivesRef.current, elective]),
  ...
  const removeCustomElective = useCallback(
    (electiveId: string) =>
      commitCustomElectives(customElectivesRef.current.filter((e) => e.id !== electiveId)),
    [commitCustomElectives],
  );
  ...
  const updateCustomElective = useCallback(
    (elective: CustomElective) =>
      commitCustomElectives(
        customElectivesRef.current.map((e) => (e.id === elective.id ? elective : e)),
      ),
    [commitCustomElectives],
  );
  ...
  const optionsByType = useMemo(() => buildOptionsByType(customElectives), [customElectives]);
  ...
  const importSettings = useCallback((jsonString: string): boolean => {
    try {
      ...
      const tileLabel: TileLabelMode = data.tileLabel === "code" ? "code" : "abbreviation";
